...
export class CardInfo extends ChildComponent {
  ...
  destroy() {
	...
	fillElements() {
		...
		$MV(this.element).find('#card-expire-date').text(this.card.expireDate)

		const cardCvcElement = $MV(this.element).find('#card-cvc')
    ...
    if (this.store.state.user) {
			$MV(this.element).html(new Loader().render().outerHTML)
